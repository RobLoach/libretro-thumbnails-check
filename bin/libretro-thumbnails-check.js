...
glob('libretro-database/rdb/*.rdb', function (err, files) {
	...
	tasks.run().then(function (report) {
		...
		for (let systemName in report) {
			...
			output += `| [${systemName}](out/${systemName.split(' ', '%20')}.txt) | ${systemReport.boxart} / ${systemReport.total} | ${systemReport.snap} / ${systemReport.total} | ${systemReport.title} / ${systemReport.total} | ${total} / ${systemReport.total * 3} | **${totalPercent}%** |\n`
		}
